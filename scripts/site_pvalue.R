...
dat$ratioPos = (1+dat$Nuclease_Position_Reads) / (1+dat$Control_Position_Reads)

# Density plot
ggplot(dat, aes(log2(ratioPos))) + geom_histogram(binwidth=0.5) + theme_bw() + scale_y_continuous(labels=comma) + ggtitle("log2((1+nuclease_pos_reads)/(1+control_pos_reads))")
